      ...
      @if (count($results) > 0)
        @foreach ($results as $index => $result)
          <tr>
            ...
            <td>{{ $result->harga }}</td>
            <td>
              <a href="{{ url('/drink/'.$result->id.'/edit/') }}" class="btn btn-sm btn-warning"><i class="fas fa-edit pr-1"></i>Edit</a>
              <form method="POST" action="{{ url('/drink/'.$result->id)}}" class="d-inline pl-2">
                @csrf
                @method('DELETE')
